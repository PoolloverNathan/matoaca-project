...
static func spawn_from_tilemap(tree, tile_spawner):
	...
	for cellv in source_tilemap.get_used_cells():
		...
		var scene_path
		if mapping_entry.has("scene"):
		# Using the entry, find the scene path for this tile name
			scene_path = mapping_entry['scene']
		else:
			var atcoord = source_tilemap.get_cell_autotile_coord(cellv.x, cellv.y)
			var key = "atlas_" + str(atcoord.x) + "_" + str(atcoord.y)
			if mapping_entry.has(key):
				scene_path = mapping_entry
			else:
				continue
